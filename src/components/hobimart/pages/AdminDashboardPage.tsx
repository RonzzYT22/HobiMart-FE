...
const COLORS = ['hobbyco-orange', '#3B82F6', '#10B981', '#8B5CF6', '#F59E0B', '#EF4444', '#EC4899', '#06B6D4'];
    ...
    <div className="min-h-screen bg-[hobbyco-cream]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-[hobbyco-dark]">Admin Dashboard</h1>
            <p className="text-gray-400 text-sm mt-1">HobiMart management</p>
          </div>
          <button onClick={() => navigate('home')} className="text-sm text-[hobbyco-orange] font-black hover:underline flex items-center gap-1">
        ...
        <div className="flex items-center gap-1 mb-6 overflow-x-auto pb-1">
          {TABS.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-black transition-all whitespace-nowrap ${
                activeTab === tab.id ? 'bg-[hobbyco-orange] text-white shadow-sm' : 'bg-white border border-[gray-200] text-gray-500 hover:border-gray-300'}`}>
              <tab.icon className="w-4 h-4" /> {tab.label}
            </button>
          ))}
          <button onClick={() => loadTab(activeTab)} className="ml-auto p-2.5 rounded-xl text-gray-400 hover:text-[hobbyco-orange] hover:bg-orange-50 transition-colors">
        ...
        {loading && (
          <div className="flex items-center justify-center py-20">
            <div className="w-10 h-10 border-4 border-[hobbyco-orange] border-t-transparent rounded-full animate-spin" />
          </div>
        )}
        ...
        {!loading && !error && (
          <>
                ...
                {categories && (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="bg-white rounded-2xl border border-[gray-200] p-6">
                      <h3 className="text-base font-black text-[hobbyco-dark] mb-4">Top Categories (Sales)</h3>
                      <div className="space-y-3">
                        {categories.sales?.slice(0, 8).map((c: any, i: number) => (
                          <div key={i} className="flex items-center justify-between">
                            <span className="text-sm text-gray-600">{c.category}</span>
                            <div className="flex items-center gap-3">
                              <div className="w-32 h-2 bg-gray-100 rounded-full overflow-hidden"><div className="h-full bg-[hobbyco-orange] rounded-full" style={{ width: `${Math.min(100, (c.total_sold / (categories.sales[0]?.total_sold || 1)) * 100)}%` }} /></div>
                              <span className="text-sm font-black text-[hobbyco-dark] w-12 text-right">{c.total_sold}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="bg-white rounded-2xl border border-[gray-200] p-6">
                      <h3 className="text-base font-black text-[hobbyco-dark] mb-4">Top Sellers (Revenue)</h3>
                      <div className="space-y-3">
                        {sellers?.byRevenue?.slice(0, 8).map((s: any, i: number) => (
                          <div key={i} className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-xs font-black">{s.name?.charAt(0)}</div>
                              <span className="text-sm font-medium text-[hobbyco-dark]">{s.name}</span>
                            </div>
                            <span className="text-sm font-black">{formatPrice(s.revenue)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            {activeTab === 'users' && (
              <div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <Button onClick={() => loadTab('users')} className="bg-[hobbyco-orange] hover:bg-[hobbyco-orange-dark] text-white rounded-xl text-sm">Search</Button>
                </div>
                <div className="bg-white rounded-2xl border border-[gray-200] overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead><tr className="text-left text-gray-400 border-b border-[gray-200] bg-gray-50">
                      ...
                      <tbody>
                        {users.map(u => (
                          <tr key={u.id} className="border-b border-[#F1F5F9] hover:bg-gray-50">
                            <td className="p-3 font-black text-[hobbyco-dark]">{u.name}</td>
                  ...
                  <div className="p-3 text-xs text-gray-400 border-t border-[gray-200]">{usersTotal} total users</div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'products' && (
              <div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <Button onClick={() => loadTab('products')} className="bg-[hobbyco-orange] hover:bg-[hobbyco-orange-dark] text-white rounded-xl text-sm">Search</Button>
                ...
                <div className="bg-white rounded-2xl border border-[gray-200] overflow-hidden">
                          ...
                          <tr key={p.id} className="border-b border-[#F1F5F9] hover:bg-gray-50">
                            <td className="p-3">
                              <div className="flex items-center gap-2">
                                ...
                                <div><p className="font-black text-[hobbyco-dark] text-xs">{p.name}</p><p className="text-[10px] text-gray-400">{p.sku}</p></div>
                              </div>
                            </td>
                            <td className="p-3 text-xs text-gray-500">{p.seller?.name || '-'}</td>
                            <td className="p-3 text-xs font-black">{formatPrice(p.price)}</td>
                  ...
                  <div className="p-3 text-xs text-gray-400 border-t border-[gray-200]">{productsTotal} total products</div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'orders' && (
              <div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <Button onClick={() => loadTab('orders')} className="bg-[hobbyco-orange] hover:bg-[hobbyco-orange-dark] text-white rounded-xl text-sm">Search</Button>
                </div>
                <div className="bg-white rounded-2xl border border-[gray-200] overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {orders.map(o => (
                          <tr key={o.id} className="border-b border-[#F1F5F9] hover:bg-gray-50">
                            <td className="p-3 font-black text-[hobbyco-dark] text-xs">{o.orderNumber}</td>
                            <td className="p-3 text-xs text-gray-500">{o.customer?.name || '-'}</td>
                            <td className="p-3 text-xs font-black">{formatPrice(o.total)}</td>
                  ...
                  <div className="p-3 text-xs text-gray-400 border-t border-[gray-200]">{ordersTotal} total orders</div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'trades' && (
              <div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <Button onClick={() => loadTab('trades')} className="bg-[hobbyco-orange] hover:bg-[hobbyco-orange-dark] text-white rounded-xl text-sm">Search</Button>
                </div>
                <div className="bg-white rounded-2xl border border-[gray-200] overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {trades.map(t => (
                          <tr key={t.id} className="border-b border-[#F1F5F9] hover:bg-gray-50">
                            <td className="p-3 text-xs font-black text-[hobbyco-dark]">#{t.id}</td>
                            <td className="p-3 text-xs text-gray-500">{t.initiator?.name}</td>
                            <td className="p-3 text-xs text-gray-500">{t.receiver?.name}</td>
                            <td className="p-3 text-xs font-black">{t.cashDifference !== 0 ? formatPrice(t.cashDifference) : 'Trade murni'}</td>
                            <td className="p-3">
                  ...
                  <div className="p-3 text-xs text-gray-400 border-t border-[gray-200]">{tradesTotal} total trades</div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'analytics' && charts && (
              <div className="space-y-6">
                <div className="bg-white rounded-2xl border border-[gray-200] p-6">
                  <h3 className="text-base font-black text-[hobbyco-dark] mb-4">GMV (6 Bulan)</h3>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={charts.gmv}>
                      ...
                      <Bar dataKey="gmv" fill="hobbyco-orange" radius={[6, 6, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="bg-white rounded-2xl border border-[gray-200] p-6">
                    <h3 className="text-base font-black text-[hobbyco-dark] mb-4">Orders per Bulan</h3>
                  ...
                  <div className="bg-white rounded-2xl border border-[gray-200] p-6">
                    <h3 className="text-base font-black text-[hobbyco-dark] mb-4">Trades per Bulan</h3>
                    <ResponsiveContainer width="100%" height={250}>
                ...
                <div className="bg-white rounded-2xl border border-[gray-200] p-6">
                  <h3 className="text-base font-black text-[hobbyco-dark] mb-4">New Users per Bulan</h3>
                  <ResponsiveContainer width="100%" height={250}>
...
function StatCard({ icon, label, value, change, color }: { icon: React.ReactNode; label: string; value: string | number; change?: number; color: string }) {
  return (
    <div className="bg-white rounded-2xl border border-[gray-200] p-5">
      ...
      <p className="text-2xl font-black text-[hobbyco-dark]">{value}</p>
      <div className="flex items-center gap-1 mt-1">
        <p className="text-xs text-gray-400">{label}</p>
        {change !== undefined && (
          <span className={`text-xs font-black ${change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            {change > 0 ? '+' : ''}{change}%
          </span>
        )}
      </div>
