      ...
      <button
        ...
        className={`w-full text-left flex items-start gap-3.5 p-4 rounded-xl transition-all duration-200 group ${
          notification.read
            ...
            : 'bg-[#FFF7ED] border border-[hobbyco-orange]/10 hover:border-[hobbyco-orange]/20'
        }`}
      >
          ...
          <div className="flex items-center gap-2">
            <h4
              className={`text-sm font-black truncate ${
                notification.read ? 'text-[hobbyco-dark]' : 'text-[hobbyco-dark]'
              }`}
            >
              {notification.title}
            </h4>
            {!notification.read && (
              <span className="w-2 h-2 rounded-full bg-[hobbyco-orange] flex-shrink-0" />
            )}
          </div>
          <p
        ...
        {/* Chevron */}
        <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-[hobbyco-orange] transition-colors flex-shrink-0 mt-2.5" />
      </button>
    );
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          <div className="flex items-center gap-2.5">
            ...
            {hasUnread && (
              <span className="inline-flex items-center justify-center min-w-[24px] h-6 px-2 rounded-full bg-[hobbyco-orange] text-white text-xs font-black">
                {unreadCount}
              </span>
            )}
          </div>
          ...
          {hasUnread && (
            <button
              onClick={markAllRead}
              className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-black text-[hobbyco-orange] bg-[hobbyco-orange]/10 rounded-xl hover:bg-[hobbyco-orange]/20 transition-colors"
            >
