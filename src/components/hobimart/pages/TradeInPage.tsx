      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-[hobbyco-orange] flex items-center justify-center">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative flex-1 min-w-[180px]">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 text-sm border border-[gray-200] rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[hobbyco-orange]/20 focus:border-[hobbyco-orange] transition-all"
              />
            </div>

            {/* Category */}
            <Select value={categoryFilter} onValueChange={setCategoryFilter}>
              <SelectTrigger className="w-[150px] h-9 text-xs rounded-lg border-[gray-200] bg-white">
                <SelectValue />
              </SelectTrigger>
            ...
            <Select value={lookingFor} onValueChange={setLookingFor}>
              <SelectTrigger className="hidden lg:block w-[160px] h-9 text-xs rounded-lg border-[gray-200] bg-white">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="rounded-lg">
            ...
            <Select value={locationFilter} onValueChange={setLocationFilter}>
              <SelectTrigger className="hidden lg:block w-[140px] h-9 text-xs rounded-lg border-[gray-200] bg-white">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="rounded-lg">
            ...
            <Select value={sortBy} onValueChange={setSortBy}>
              <SelectTrigger className="w-[130px] lg:w-[150px] h-9 text-xs rounded-lg border-[gray-200] bg-white">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="rounded-lg">
                {sortOptions.map((opt) => (
            ...
            <Sheet>
              <SheetTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="lg:hidden gap-1.5 rounded-lg text-xs font-medium border-[gray-200] hover:bg-orange-50 hover:border-[hobbyco-orange] hover:text-[hobbyco-orange] h-9"
                >
                  <SlidersHorizontal className="w-3.5 h-3.5" />
                  Filters
                  {activeFilterCount > 0 && (
                    <span className="w-5 h-5 bg-[hobbyco-orange] text-white text-[10px] font-black rounded-full flex items-center justify-center">
                      {activeFilterCount}
                    </span>
                  )}
                </Button>
              ...
              <SheetContent side="left" className="w-80 p-0 bg-white">
                <SheetHeader className="border-b border-[gray-200] px-5 py-4">
                  <SheetTitle className="text-[hobbyco-dark]">Trade Filters</SheetTitle>
                </SheetHeader>
                <div className="px-5 py-5 space-y-5">
                  <div>
                    ...
                    <Select value={lookingFor} onValueChange={setLookingFor}>
                      <SelectTrigger className="w-full h-9 text-sm rounded-lg border-[gray-200]">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="rounded-lg">
                        {lookingForOptions.map((opt) => (
                  ...
                  {activeFilterCount > 0 && (
                    <button
                      ...
                      className="w-full py-2.5 text-sm font-black text-red-500 border border-red-200 rounded-xl hover:bg-red-50 transition-colors"
                    >
                      Clear All Filters
                    </button>
                  )}
            ...
            {activeFilterCount > 0 && (
              <button
                onClick={clearFilters}
                className="hidden lg:block text-xs text-red-500 font-black hover:underline whitespace-nowrap"
              >
                Clear All
              </button>
            )}
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="text-center py-16 sm:py-24 bg-white rounded-2xl border border-[gray-200]">
            ...
            <button
              onClick={clearFilters}
              className="px-5 py-2.5 bg-[hobbyco-orange] text-white text-sm font-black rounded-xl hover:bg-[hobbyco-orange-dark] active:scale-[0.98] transition-all shadow-lg shadow-orange-500/20"
            >
              Clear All Filters
            </button>
          </div>
        ...
        <section className="mt-14">
          <h2 className="text-xl sm:text-2xl font-black text-[hobbyco-dark] text-center mb-2">
            How Trading Works
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {/* Step 1 */}
            <div className="relative bg-white rounded-2xl border border-[gray-200] p-6 text-center group hover:shadow-lg hover:shadow-teal-500/5 hover:-translate-y-1 transition-all duration-300">
              ...
              <span className="absolute top-4 right-5 w-7 h-7 rounded-full bg-teal-500 text-white text-xs font-black flex items-center justify-center">
                1
              </span>
              <h3 className="text-base font-black text-[hobbyco-dark] mb-1.5">
                Find an Item
              </h3>
            ...
            {/* Step 3 */}
            <div className="bg-white rounded-2xl border border-[gray-200] p-6 text-center group hover:shadow-lg hover:shadow-teal-500/5 hover:-translate-y-1 transition-all duration-300">
              <div className="w-14 h-14 rounded-2xl bg-teal-50 flex items-center justify-center mx-auto mb-4 group-hover:bg-teal-100 transition-colors">
