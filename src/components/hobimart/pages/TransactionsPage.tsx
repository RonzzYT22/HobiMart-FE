...
export default function TransactionsPage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    const Icon = cfg.icon;
    return <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-bold ${cfg.color}`}>
    ...
    <div className="min-h-screen bg-hobbyco-cream">
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <Breadcrumb>
            <BreadcrumbList>
              <BreadcrumbItem>
                <BreadcrumbLink onClick={() => navigate('home')} className="cursor-pointer text-gray-500 hover:text-hobbyco-orange transition-colors font-medium">Home</BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator className="text-gray-200" />
              <BreadcrumbItem>
                <BreadcrumbPage className="text-hobbyco-dark font-bold">Riwayat Transaksi</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-6">
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-hobbyco-dark font-display">Riwayat Transaksi</h1>
            <p className="text-sm text-gray-500 mt-1 font-medium">{transactionsTotal} total transaksi</p>
          </div>
          <Button onClick={handleExport} variant="outline" className="rounded-xl text-sm border-gray-200 hover:bg-hobbyco-cream hover:border-hobbyco-orange">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-6">
          ...
          <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-xl p-1">
            {TYPES.map(t => (
              <button key={t.value} onClick={() => { setTypeFilter(t.value); setPage(1); }}
                className={`flex items-center gap-1 px-3.5 py-2 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${
                  typeFilter === t.value ? 'bg-hobbyco-orange text-white shadow-sm' : 'text-gray-500 hover:text-hobbyco-dark'}`}>
          ...
          <div className="relative flex-1">
            ...
            <Input placeholder="Cari transaksi..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              className="pl-9 h-10 rounded-xl border-gray-200 bg-white text-sm focus-visible:border-hobbyco-orange" />
          </div>
          {/* Status filter */}
          <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setPage(1); }}
            className="px-3 py-2 text-sm border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-hobbyco-orange/20">
          ...
          {transactions.length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-100 py-16 text-center">
              <div className="w-20 h-20 rounded-full bg-hobbyco-cream flex items-center justify-center mx-auto mb-5">
                <ShoppingBag className="w-10 h-10 text-hobbyco-green/40" />
              </div>
              <h3 className="text-lg font-black text-hobbyco-dark">Belum ada transaksi</h3>
              <p className="text-sm text-gray-400 mt-1.5 font-medium">Riwayat pembelian dan trade akan muncul di sini</p>
            </div>
          ) : (
            transactions.map((tx: any) => (
              <div key={tx.id} className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5 card-hover">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    {/* Icon */}
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                      tx.type === 'trade' ? 'bg-purple-50 text-purple-600' : 'bg-orange-50 text-hobbyco-orange'}`}>
                      {tx.type === 'trade' ? <ArrowLeftRight className="w-5 h-5" /> : <ShoppingBag className="w-5 h-5" />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-hobbyco-dark">
        ...
        {hasMore && (
          <div className="flex justify-center mt-6">
            <Button variant="outline" onClick={() => setPage(p => p + 1)} className="rounded-xl text-sm border-gray-200 hover:bg-hobbyco-cream hover:border-hobbyco-orange">
              Load More
            </Button>
          </div>
        )}
