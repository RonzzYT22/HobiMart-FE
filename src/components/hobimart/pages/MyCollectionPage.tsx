        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbPage className="text-[hobbyco-dark] font-black">My Collection</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
          ...
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-2xl sm:text-3xl font-black text-[hobbyco-dark]">
                <Sparkles className="w-6 h-6 text-[hobbyco-orange] inline" /> My Collection
              </h1>
              <span className="inline-flex items-center justify-center min-w-7 h-7 rounded-full bg-[hobbyco-orange] text-white text-xs font-black px-2">
                {collectionProducts.length} Items
              </span>
            </div>
            <p className="text-sm text-[gray-500] mt-1">Your personal hobby collection showcase</p>
          </div>
          <Button onClick={() => { resetForm(); setShowForm(!showForm); }}
            className="bg-[hobbyco-orange] hover:bg-[hobbyco-orange-dark] text-white font-black rounded-xl text-sm px-5 py-2.5">
        ...
        {showForm && (
          <div className="bg-white rounded-2xl border border-[gray-200] p-5 sm:p-6 mb-6">
            <h3 className="text-base font-black text-[hobbyco-dark] mb-4">
              {editingId ? 'Edit Collection Item' : 'Add New Collection Item'}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {/* Product ID */}
              <div>
                <label className="block text-xs font-black text-gray-500 mb-1">Product ID (dari katalog)</label>
              ...
              {/* Condition */}
              <div>
                <label className="block text-xs font-black text-gray-500 mb-1">Condition</label>
                <select value={form.condition} onChange={e => setForm(f => ({ ...f, condition: e.target.value }))}
                  className="w-full px-3 py-2 text-sm border border-[gray-200] rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-[hobbyco-orange]/20">
                  {CONDITIONS.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              {/* Grade */}
              <div>
                <label className="block text-xs font-black text-gray-500 mb-1">Grade (optional)</label>
                <Input value={form.grade} onChange={e => setForm(f => ({ ...f, grade: e.target.value }))}
                  placeholder="e.g. PSA 10, BGS 9.5" className="rounded-xl" />
              </div>
              {/* Purchase Price */}
              <div>
                <label className="block text-xs font-black text-gray-500 mb-1">Purchase Price (Rp)</label>
              ...
              {/* Purchase Date */}
              <div>
                <label className="block text-xs font-black text-gray-500 mb-1">Purchase Date</label>
              ...
              <div className="flex items-center gap-3 pt-5">
                <button onClick={() => setForm(f => ({ ...f, is_public: !f.is_public }))}
                  className={`w-11 h-6 rounded-full transition-colors ${form.is_public ? 'bg-[hobbyco-orange]' : 'bg-gray-300'}`}>
              ...
              <div className="sm:col-span-2">
                <label className="block text-xs font-black text-gray-500 mb-1">Images</label>
                <div className="flex items-center gap-2">
                  <label className="cursor-pointer px-4 py-2 border border-dashed border-[gray-200] rounded-xl text-sm text-gray-500 hover:border-[hobbyco-orange] hover:text-[hobbyco-orange] transition-colors">
            ...
            <div className="flex gap-3 mt-5">
              <Button onClick={handleSubmit} disabled={loading} className="bg-[hobbyco-orange] hover:bg-[hobbyco-orange-dark] text-white font-black rounded-xl">
                <Save className="w-4 h-4 mr-1" /> {loading ? 'Saving...' : 'Save'}
              </Button>
              <Button onClick={() => { setShowForm(false); resetForm(); }} variant="outline" className="rounded-xl border-[gray-200]">Cancel</Button>
            </div>
          </div>
        )}
          ...
          {categories.map((cat) => (
            <button key={cat.name} onClick={() => setSelectedCategory(selectedCategory === cat.name ? 'All' : cat.name)}
              className={`relative overflow-hidden rounded-2xl border p-5 text-left transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 group ${
                selectedCategory === cat.name ? 'border-[hobbyco-orange] bg-[hobbyco-orange]/5 ring-1 ring-[hobbyco-orange]/20' : 'border-[gray-200] bg-white hover:border-gray-300'}`}>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-black text-[hobbyco-dark]">{cat.name}</p>
                  <p className={`text-2xl font-black mt-0.5 ${cat.textColor}`}>{cat.count}<span className="text-xs font-normal text-gray-400 ml-1">items</span></p>
                </div>
              </div>
              <div className={`absolute bottom-0 left-0 right-0 h-1 bg-[hobbyco-orange] transition-all ${selectedCategory === cat.name ? 'opacity-100' : 'opacity-0'}`} />
            </button>
          ))}
        </div>
            ...
            {['All', ...categories.map(c => c.name)].map(cat => (
              <button key={cat} onClick={() => setSelectedCategory(cat)}
                className={`px-3.5 py-2 rounded-xl text-xs font-black transition-all whitespace-nowrap ${
                  selectedCategory === cat ? 'bg-[hobbyco-orange] text-white shadow-sm' : 'bg-white border border-[gray-200] text-gray-500 hover:border-gray-300'}`}>
                {cat}
              </button>
            ))}
          </div>
        ...
        {filteredItems.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
            {filteredItems.map((item: any) => (
              <div key={item.id} className="bg-white rounded-2xl border border-[gray-200] overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1 group cursor-pointer">
                <div className="relative aspect-[4/3] bg-gray-100 overflow-hidden">
                  ...
                  <div className="absolute bottom-2.5 left-2.5">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-black ${getConditionColor(item.condition)}`}>
                      ● {item.condition}
                    </span>
                  </div>
                  {!item.is_public && (
                ...
                <div className="p-3.5">
                  ...
                  <h3 className="text-sm font-black text-[hobbyco-dark] mt-0.5 line-clamp-1 group-hover:text-[hobbyco-orange] transition-colors">
                    {item.product?.name || 'Unknown'}
                  </h3>
                  {item.grade && <p className="text-xs text-[hobbyco-orange] font-black mt-1">{item.grade}</p>}
          ...
          <div className="bg-white rounded-2xl border border-[gray-200] py-16 text-center">
            ...
            <h3 className="text-lg font-black text-[hobbyco-dark]">No items found</h3>
            ...
            <Button onClick={() => { setSearchQuery(''); setSelectedCategory('All'); }} variant="outline" className="mt-5 rounded-xl font-black text-sm">
