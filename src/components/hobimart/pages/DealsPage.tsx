    ...
    <section>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-lg sm:text-xl font-black text-hobbyco-dark font-display">{title}</h2>
        </div>
        {viewAllLabel && (
          <button className="text-xs font-bold text-hobbyco-orange hover:text-hobbyco-orange-dark flex items-center gap-1 transition-colors">
            {viewAllLabel}
...
function BundleCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden cursor-pointer transition-all duration-300 card-hover group">
      <div className="relative aspect-[4/3] bg-gradient-to-br from-hobbyco-green to-hobbyco-green-light overflow-hidden">
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            <span className="text-5xl drop-shadow-lg">{image}</span>
            <p className="text-xs text-white/90 font-bold mt-2 drop-shadow">{title}</p>
          </div>
        </div>
        <div className="absolute top-2.5 left-2.5 flex flex-col gap-1.5">
          <span className="px-2.5 py-0.5 bg-hobbyco-dark text-white text-[10px] font-bold rounded-md flex items-center gap-1 badge-comic">
      ...
      <div className="p-3.5">
        <h3 className="text-sm font-black text-hobbyo-dark group-hover:text-hobbyco-orange transition-colors line-clamp-1">{title}</h3>
        <p className="text-[11px] text-gray-400 mt-0.5 line-clamp-1">
          {items.length} items included
        </p>
        <div className="flex items-end gap-2 mt-2">
          <p className="text-base font-black text-hobbyco-dark">{formatPrice(bundlePrice)}</p>
          <p className="text-xs text-gray-400 line-through">{formatPrice(originalTotal)}</p>
        </div>
        <button className="w-full mt-3 py-2 bg-hobbyco-orange text-white text-xs font-bold rounded-xl hover:bg-hobbyco-orange-dark active:scale-[0.98] transition-all btn-hover-lift flex items-center justify-center gap-1.5 shadow-md shadow-hobbyco-orange/20">
    ...
    <div className="min-h-screen bg-hobbyco-cream">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
        ...
        <div className="relative mb-8 overflow-hidden rounded-2xl">
          <div className="bg-hero-gradient p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 relative overflow-hidden">
            {/* Decorative circles */}
            ...
            <div className="absolute top-1/2 right-1/4 w-32 h-32 bg-white/8 rounded-full"></div>
            ...
            <div className="relative z-10 flex-1">
              ...
              <h2 className="text-xl sm:text-2xl lg:text-3xl font-black text-white leading-tight font-display">
                Summer Hobby Festival!
              </h2>
              <p className="text-white/70 text-sm mt-1.5 max-w-lg font-medium">
                Use code <span className="font-black text-yellow-300">HOBBYCO10</span> for extra 10%
            ...
            <div className="relative z-10 flex items-center gap-6 sm:gap-8">
              ...
              <div className="text-center">
                <p className="text-2xl sm:text-3xl font-black text-yellow-300">70%</p>
        ...
        <div className="space-y-10">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2.5">
                ...
                <h2 className="text-lg sm:text-xl font-black text-hobbyco-dark font-display">
                  ⚡ Flash Sale
                </h2>
              </div>
              <button className="text-xs font-bold text-hobbyco-orange hover:text-hobbyco-orange-dark flex items-center gap-1 transition-colors">
            ...
            <div className="flex items-center gap-3 mb-4 px-4 py-3 bg-red-50 border border-red-200/60 rounded-xl">
              ...
              <span className="text-xs text-red-600 font-bold">Ends in:</span>
              <div className="flex items-center gap-1.5">
                {[
                  ...
                  <div key={unit.label} className="flex items-center gap-1">
                    <span className="inline-flex items-center justify-center min-w-[32px] h-8 px-2 bg-hobbyco-dark text-white text-sm font-bold rounded-lg">
                      {unit.value}
                    </span>
                    {unit.label !== 'S' && (
                      <span className="text-hobbyco-dark font-bold text-sm">:</span>
                    )}
                  </div>
                ))}
              </div>
          ...
          {/* ── Bottom CTA ── */}
          <div className="bg-hero-gradient rounded-2xl p-6 sm:p-8 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              <Sparkles className="w-8 h-8 text-hobbyco-orange mx-auto mb-3 animate-sparkle" />
              <h3 className="text-xl sm:text-2xl font-black text-white font-display">
                Don&apos;t Miss Out!
              </h3>
              <p className="text-sm text-white/60 mt-2 max-w-md mx-auto font-medium">
                New deals are added every week. Follow our store and enable notifications
                to be the first to grab limited offers.
              </p>
              <button className="mt-5 px-6 py-2.5 bg-hobbyco-orange text-white text-sm font-bold rounded-xl hover:bg-hobbyco-orange-dark active:scale-[0.98] transition-all shadow-orange-glow btn-hover-lift inline-flex items-center gap-2">
